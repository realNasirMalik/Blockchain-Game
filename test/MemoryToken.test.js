...
contract('Memory Token', (accounts) => {
     ...
     describe('token distribution', async () => {
          ...
          it('mints tokens', async () => {
               ...
               // It should increase the total total supply
               result = await token.totalSupply()
               assert.equal(result.toString(), '1, total supply is correct')

               // It increments owner balance
               result = await token.balanceOf(account[0])
               assert.equal(result.totString(), accounts[0].toString(), 'ownerOf is correct')
               ...
               let expected = ['1']
               assert.equal(token.Ids.toString(), expected.toString(), 'tokenIds are correct')
